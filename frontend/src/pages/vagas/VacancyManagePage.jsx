...
export default function VacancyManagePage() {
  ...
  if (state.status === 'loading') content = <section className="management-state" role="status"><p>Carregando gestão da vaga…</p></section>;
    ...
    content = <><header className="management-page-header management-page-header--actions"><div><p className="management-eyebrow">Detalhe do proprietário</p><h1>{vacancy.titulo}</h1><div className="management-title-status"><VacancyStatusBadge status={vacancy.status} /><span>{vacancy.categoria || vacancy.tipoContrato}</span></div></div><a className="btn management-button-secondary" href="/minhas-vagas">Voltar</a></header>{feedback ? <p className="management-feedback management-feedback--success" role="status">{feedback}</p> : null}{actionError ? <p className="management-feedback management-feedback--error" role="alert">{actionError}</p> : null}<div className="management-detail-layout"><article className="management-card management-detail"><div className="management-detail__actions"><a className="btn management-button-secondary" href={`/candidatos-vaga.html?vaga=${encodeURIComponent(id)}`}>Candidaturas recebidas</a><a className="btn management-button-secondary" href={`/vagas/${id}/editar`}>Editar vaga</a>{actions.map((item) => <button className="btn btn--primario" key={item.action} type="button" disabled={Boolean(activeAction)} onClick={() => runStatusAction(item.action)}>{activeAction === item.action ? 'Processando…' : item.label}</button>)}{cancelable ? <button className="btn management-button-danger" id="cancelar" type="button" disabled={Boolean(activeAction)} onClick={() => setCancelOpen(true)}>Cancelar vaga</button> : null}</div>{actions.length === 0 ? <p className="management-final-state">Este é um estado final; nenhuma nova transição está disponível.</p> : null}<dl className="management-detail__facts"><div><dt>Contratante</dt><dd>{vacancy.nomeContratante}</dd></div><div><dt>Local</dt><dd>{vacancy.cidade}/{vacancy.estado}</dd></div><div><dt>Modelo</dt><dd>{vacancy.modeloTrabalho}</dd></div><div><dt>Contrato</dt><dd>{vacancy.tipoContrato}</dd></div><div><dt>Remuneração</dt><dd>{vacancyRemuneration(vacancy)}</dd></div><div><dt>Prazo</dt><dd>{vacancy.dataLimiteCandidatura || 'Não informado'}</dd></div></dl><section><h2>Descrição</h2><p className="management-preserve-text">{vacancy.descricao}</p></section><section><h2>Requisitos</h2><p className="management-preserve-text">{vacancy.requisitos}</p></section>{vacancy.beneficios ? <section><h2>Benefícios</h2><p className="management-preserve-text">{vacancy.beneficios}</p></section> : null}{vacancy.funcaoIds?.length ? <section><h2>Funções</h2><div className="management-tag-list">{vacancy.funcaoIds.map((tagId) => <span key={tagId}>Função #{tagId}</span>)}</div></section> : null}</article><RelatedVacancies vacancies={related} /></div><VacancyCancelDialog open={cancelOpen} vacancyTitle={vacancy.titulo} onClose={() => setCancelOpen(false)} onConfirm={cancel} /></>;
  }
